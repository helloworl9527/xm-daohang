...
export function DirectoryShell({ children }: { children: ReactNode }) {
  ...
  function commit(event?: FormEvent) { event?.preventDefault(); const normalized = (input.current?.value ?? draft).normalize("NFKC").trim(); if (!normalized || normalized.length > 100 || /[\u0000-\u001f\u007f-\u009f]/u.test(normalized)) { setError(t("invalid")); input.current?.focus(); return; } setError(null); router.push(`${pathname}?q=${encodeURIComponent(normalized)}`); }
  function commitOnEnter(event: KeyboardEvent<HTMLInputElement>) { if (event.key !== "Enter") return; event.preventDefault(); commit(); }
  function clear() { setDraft(""); setError(null); router.push(pathname); input.current?.focus(); }
  return <>
    <div className="directory-title-row"><h1>{t("title")}</h1><KeywordSearch draft={draft} error={error} inputRef={input} onChange={(value) => { setDraft(value); setError(null); }} onClear={clear} onEnter={commitOnEnter} onSubmit={commit} /></div>
    <MotionRegion className="directory-content">{!committed ? children : state.kind === "loading" ? <section aria-busy="true" aria-label={t("searching")} className="directory-grid directory-skeleton">{[0,1,2].map((i) => <div key={i}><span /><span /><span /></div>)}</section> : state.kind === "error" ? <DirectoryState kind="error" onRetry={() => setRetry((value) => value + 1)} search /> : state.kind === "success" ? <section aria-live="polite" className="search-results"><header><div><h2>{state.matches.length ? t("results") : t("noResults")}</h2><p>{t("resultMeta", { query: state.query, count: state.matches.length })}</p></div><Pressable onClick={clear} type="button">{t("clear")}</Pressable></header>{state.matches.length ? <SearchCards matches={state.matches} /> : <p className="directory-empty">{t("noResultsDetail")}</p>}</section> : null}</MotionRegion>
  </>;
}
