...
describe("Telegram processing commands", () => {
  async function seedItem(id: string, status: "completed" | "processing" | "failed") {
    await db.insert(items).values({
      id,
      url: `https://example.com/${id.slice(0, 8)}`,
      urlCanonical: `https://example.com/${id.slice(0, 8)}`,
      type: "web",
      source: "telegram",
      status,
      title: status === "completed" ? "旧标题" : null,
      summary: status === "completed" ? "这是用于重抓的中文总结。" : null,
      tags: status === "completed" ? ["重抓", "测试", "Telegram"] : [],
    });
  }

  it("runs refetch and retry by allowlisted short ID", async () => {
    await seedItem("aaaaaaaa-0000-4000-8000-000000000001", "completed");
    await seedItem("bbbbbbbb-0000-4000-8000-000000000002", "failed");
    const send = vi.fn(async () => undefined);
    await handleTelegramMessage({ senderId: 42, chatId: "4200", text: "/refetch aaaaaaaa" }, { send });
    await handleTelegramMessage({ senderId: 42, chatId: "4200", text: "/retry bbbbbbbb" }, { send });
    expect(await db.select().from(processingRequests)).toHaveLength(2);
    expect(send).toHaveBeenCalledWith("4200", "已开始重新抓取。");
  });

  it("does not leak unknown IDs and rejects an item already processing", async () => {
    await seedItem("cccccccc-0000-4000-8000-000000000003", "processing");
    await pool.query(
      `insert into processing_requests (item_id, process_generation, emb_version, attempt, status)
       values ('cccccccc-0000-4000-8000-000000000003', 0, 1, 0, 'pending')`,
    );
    const send = vi.fn(async () => undefined);
    await handleTelegramMessage({ senderId: 42, chatId: "4200", text: "/retry deadbeef" }, { send });
    await handleTelegramMessage({ senderId: 42, chatId: "4200", text: "/refetch cccccccc" }, { send });
    expect(send).toHaveBeenNthCalledWith(1, "4200", "未找到该条目。");
    expect(send).toHaveBeenNthCalledWith(2, "4200", "该条目正在处理中。");
    expect(await db.select().from(processingRequests)).toHaveLength(1);
  });

  it("does not downgrade a malformed processing command into a library question", async () => {
    const retrieve = vi.fn();
    const answer = vi.fn();
    const send = vi.fn(async () => undefined);
    await handleTelegramMessage(
      { senderId: 42, chatId: "4200", text: "/refetch not-a-short-id" },
      { send, retrieve, answer },
    );
    expect(retrieve).not.toHaveBeenCalled();
    expect(answer).not.toHaveBeenCalled();
    expect(send).toHaveBeenCalledWith("4200", "未找到该条目。");
  });
});
