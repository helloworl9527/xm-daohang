...
收藏系统用于保存公开网页、文本/PDF 文档和公开 GitHub 仓库。后台 worker 会抓取正文、生成中文总结与标签并写入向量；公开端按分类展示站点目录，提供字面关键词搜索并基于完整收藏库回答问题。管理端提供条目、分类、AI 分类建议、模型、定时重抓、公开额度、Telegram 与安全设置。系统面向单机 VPS 和几百条内容的规模设计，数据库、队列与向量检索统一使用 PostgreSQL。
