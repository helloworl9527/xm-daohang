...
收藏系统用于保存公开网页、文本/PDF 文档和公开 GitHub 仓库。后台 worker 仅在新网址首次入库时抓取正文、生成中文总结与标签并写入向量；公开端按分类展示站点目录，提供字面关键词搜索并基于完整收藏库回答问题。管理端提供条目、分类、AI 分类建议、模型、公开额度、Telegram 与安全设置。系统面向单机 VPS 和几百条内容的规模设计，数据库、队列与向量检索统一使用 PostgreSQL。
...
- 收藏库管理：新网址首次入库时解析，重复网址不重复处理，并支持筛选、编辑人工总结和删除。
- 分类工作台：维护固定分类、审核 AI 分类差异、保护人工分类并持久化重分类进度。
- 公开目录与字面搜索：展示全部分类和 eligible 站点，按标题、摘要及标签进行非 AI 子串匹配。
...
│   ├── app/                 # 公开端、管理端与健康检查
│   ├── db/                  # Schema、客户端与迁移
│   ├── lib/                 # 认证、抓取、AI、分类、检索、限流等服务
│   ├── messages/            # 中英文字典
│   └── worker/              # 队列、每日轮换与 Telegram
└── tests/
    ├── e2e/
    ├── integration/
    └── unit/
